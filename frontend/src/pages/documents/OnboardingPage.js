...
export default function OnboardingPage() {
  ...
  const load = async () => {
    ...
    try {
      const res = await get("/documents/onboarding/cases?status=open");
  ...
  const startOnboarding = async () => {
    if (!employee.firstName || !employee.lastName || !employee.email) {
      toast.error("Veuillez renseigner les infos collaborateur (nom, prénom, email)");
      return;
    }
    ...
    try {
      ...
      await post("/documents/onboarding/start", payload);
  ...
  const toggleChecklist = (it) => {
    setChecklist((prev) => prev.map(x => (x.key === it.key ? { ...x, done: !x.done } : x)));
  };

  const onboardingOpen = useMemo(() => cases.filter(c => c.status !== "closed"), [cases]);
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={load}><DownloadCloud className="w-4 h-4 mr-2" /> Rafraîchir</Button>
        </div>
      </div>
      ...
      <Card>
        <CardContent className="p-4 text-sm text-gray-700">
          <div className="flex items-center gap-2">
            ...
            <span>
              Pack Sénégal : Contrat (CDI/CDD/Stage), IPRES, CSS, registre du personnel, check-list matériel, OTP pour signatures.
            </span>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><Rocket className="w-5 h-5" /> Démarrer un onboarding</CardTitle>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
            <div>
              <label className="text-sm text-gray-600">Prénom *</label>
              <input className="w-full mt-1 border rounded-lg px-3 py-2" value={employee.firstName} onChange={e => setEmployee(s => ({ ...s, firstName: e.target.value }))} />
            </div>
            <div>
              <label className="text-sm text-gray-600">Nom *</label>
              <input className="w-full mt-1 border rounded-lg px-3 py-2" value={employee.lastName} onChange={e => setEmployee(s => ({ ...s, lastName: e.target.value }))} />
            </div>
            <div>
              <label className="text-sm text-gray-600">Email *</label>
              <input className="w-full mt-1 border rounded-lg px-3 py-2" type="email" value={employee.email} onChange={e => setEmployee(s => ({ ...s, email: e.target.value }))} />
            </div>
            <div>
              <label className="text-sm text-gray-600">Poste</label>
              <input className="w-full mt-1 border rounded-lg px-3 py-2" value={employee.position} onChange={e => setEmployee(s => ({ ...s, position: e.target.value }))} />
            </div>
          </div>
          ...
          <div className="flex justify-end">
            <Button onClick={startOnboarding} disabled={creating} className="bg-emerald-600 hover:bg-emerald-700">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><Users className="w-5 h-5" /> Dossiers ouverts</CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
              ...
              {onboardingOpen.map(c => {
                ...
                return (
                  <div key={c.id} className="border rounded-lg p-4 hover:shadow-sm">
                    <div className="flex items-center justify-between">
                      <div>
                        <div className="font-semibold">{c.employee?.firstName} {c.employee?.lastName}</div>
                        <div className="text-xs text-gray-500">{c.employee?.email} • {c.employee?.position || "—"}</div>
                      </div>
                      <Badge className="bg-blue-50 text-blue-700 border border-blue-200">{`${idx + 1}/${STEPS.length}`}</Badge>
                    </div>
                    <div className="mt-3">
