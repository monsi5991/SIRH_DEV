...
export default function EventFormDialog({ open, onClose, onSubmit }) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent>
        <DialogHeader><DialogTitle>Nouvel évènement</DialogTitle></DialogHeader>
        <form onSubmit={submit} className="space-y-3">
          <input className="w-full border rounded-lg px-3 py-2" placeholder="Titre" value={form.title} onChange={(e) => set("title", e.target.value)} />
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <select className="border rounded-lg px-3 py-2" value={form.type} onChange={(e) => set("type", e.target.value)}>
              <option value="meeting">meeting</option><option value="training">training</option><option value="deadline">deadline</option><option value="other">other</option>
            </select>
          </div>
          ...
          <textarea className="w-full border rounded-lg px-3 py-2" rows={3} placeholder="Description" value={form.description} onChange={(e) => set("description", e.target.value)} />
          <input className="w-full border rounded-lg px-3 py-2" placeholder="Participants (emails séparés par ,)" value={form.attendees} onChange={(e) => set("attendees", e.target.value)} />
